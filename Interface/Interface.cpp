    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                this->ShowSpecificUserSessions();
                break;
            case 5:
                this->ShowSpecificCarSessions();
                break;
            case 6:
                this->AddNewCar();
                break;
            case 7:
                this->DeleteSpecificCar();
                break;
            case 8:
                this->RefreshAllCars();
                return;
...
void Interface :: ShowAllUsers() {
    ...
    while (file.read((char *) &user, sizeof(User))) {
        ...
        wcout << '|' << setw(15) << left << user.GetName() << '|' << setw(15) << user.GetSurname() << '|' << setw(10)
              << user.GetAge() << '|'
              << setw(15) << user.GetCount() << '|' << setw(15) << user.GetSpendMoney() << '|' << setw(10)
              << user.GetStatus() << '|' << setw(15) << user.GetPhoneNumber() << '|' << setw(15)
              << user.GetPassportNum() << '|' << endl;
        cout
                << "----------------------------------------------------------------------------------------------------------------------------------"
                << endl;
    }
...
void Interface :: ShowAllSessions() {
    ...
    cout
            << "-----------------------------------------------------------------------------------------------------------------------"
            << endl;
    wcout << '|' << setw(10) << left << L"№" << '|' << setw(15) << L"Фамилия" << '|' << setw(15) << L"Имя" << '|'
          << setw(15) << L"Марка" << '|' << setw(15) << L"Модель" << '|'
        ...
        for (auto it1 = allSessions.begin(); it1 != allSessions.end(); ++it1) {
            for (auto it2 = petrolCars.begin(); it2 != petrolCars.end(); ++it2) {
                if ((it2->GetID() == it1->carID) && (it->GetID() == it1->userID)) {
                    wcout << '|' << setw(10) << left << i << '|' << setw(15) << it->GetName() << '|' << setw(15)
                          << it->GetSurname() << '|' << setw(15) << it2->GetBrand() << '|' << setw(15)
                          << it2->GetModel() << '|';
                    cout << setw(2) << left << it1->DateFrom.day << '.' << setw(2) << it1->DateFrom.month << '.'
                         << setw(9) << it1->DateFrom.year << '|' << setw(2) << it1->DateTo.day << '.' << setw(2)
                         << it1->DateTo.month << '.' << setw(9) << it1->DateTo.year << '|' << setw(10) << it1->cost
                         << '|' << endl;
                    cout
                            << "-----------------------------------------------------------------------------------------------------------------------"
                            << endl;
        ...
        if (it->carID == carID) {
            for (auto it1 = users.begin(); it1 != users.end(); ++it1) {
                if (it->userID == it1->GetID()) {
                    wcout << '|' << setw(10) << left << i << '|' << setw(15) << it1->GetName() << '|' << setw(15)
                          << it1->GetSurname() << '|';
                    cout << setw(2) << left << it->DateFrom.day << '.' << setw(2) << it->DateFrom.month << '.'
                         << setw(9) << it->DateFrom.year << '|' << setw(2) << it->DateTo.day << '.' << setw(2)
                         << it->DateTo.month << '.' << setw(9) << it->DateTo.year << '|' << setw(10) << it->cost << '|'
                         << endl;
                    cout << "---------------------------------------------------------------------------------------"
                         << endl;
                    i++;
